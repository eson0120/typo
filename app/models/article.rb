...
class Article < Content
  ...
  def initialize(*args)
    ...
    if user.admin?
    	#a2 = Article.find_by_id(other_article_id)
			self.body = self.body + Article.find_by_id(other_article_id).body
			self.comments = self.comments << Article.find_by_id(other_article_id).comments
      #puts "***********************************************************************************************************************"
      #puts self.comments
			self.save!
